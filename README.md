...
- `app/tracking/` — ByteTrack wrapper, gives objects persistent IDs
- `app/analytics/` — ROI/line-crossing math + per-track position history
- `app/events/` — the actual rules (zone entry, line crossing, loitering, count)
- `app/video/` — handles file / webcam / RTSP input
- `app/utils/` — config loading, logging, performance timing
- `config/config.yaml` — everything tunable lives here, not hardcoded in code
- `data/input/` — put your video here
- `data/output/` — logs + event CSV get written here
